...
#define SQ_SIZE (128 * 1024) //128 kb
...
int get_packet(unsigned char *packet)
{
    ...
    len = PKT_LEN;
    for (i = 2; i < len; i++) // 2 : 0 , 1 for number 
        packet[i] = next_char();
...
void put_packet(unsigned char *packet, int len)
{
    ...
    if (now - last_ts > 2000 && now > ts0 + 2000) { // over 2s
        double bps;
...
int wait_for_event(int *arg)
{
    ...
    for (;;) {
        ...
        nfds = (int)(sock + 1);
        if (select(nfds, &rfd, &wfd, 0, &tm) < 0) // non block polling 
            ABORT("system select()");
